...
| 类别 | 功能 | 参数调节 |
|------|------|---------|
| **去噪滤波** | 均值滤波、高斯滤波、中值滤波、双边滤波 | ✅ 滤波核大小（1~31） |
| **图像增强** | 直方图均衡化、CLAHE（对比度限制可调） | ✅ CLAHE clip（1.0~5.0） |
| **图像锐化** | 拉普拉斯锐化、Unsharp Mask 锐化 | ✅ 锐化强度 |
| **边缘检测** | Sobel 算子、Canny 算子（双阈值可调） | ✅ 低/高阈值（0~255） |
| **形态学操作** | 腐蚀、膨胀、开运算、闭运算 | ✅ 结构元素大小（1~31） |
...
| 功能 | 算法 | 色彩空间 | 输入参数 |
|------|------|---------|---------|
| **人脸磨皮** | 双边滤波保边去噪 + Y通道亮度提升 + 与原图加权融合 | BGR → YCrCb → BGR | `smooth_strength` (1~31, 默认15), `brightness` (-50~50, 默认20) |
| **肤色柔和** | HSV 饱和度调整：降低饱和度方差，混合平均饱和度 | BGR → HSV → BGR | `softness` (0.0~1.0, 默认0.5) |
| **对比度增强** | 线性变换 `dst = src × alpha + beta` | —（全图操作） | `alpha` (0.5~2.0, 默认1.2), `beta` (-50~50, 默认20) |
| **眼部增强** | 根据人脸 ROI 估算眼睛区域（上半部 20%~50%），Y 通道亮度提升 | BGR → YCrCb → BGR | `enhancement` (0.0~1.0, 默认0.3) |
